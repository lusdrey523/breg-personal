...
function initCategorias() {
  console.log("CATEGORIAS CARGANDO");
  categoria.innerHTML = "";
...
function calcularHoras() {
  ...
  horasCalc.textContent = "Horas: " + total.toFixed(2);
  return total;
}
...
function bindEvents() {
  categoria.onchange = () => {
    console.log("Cambio categoria:", categoria.value);
    cargarTipos();
  };
...
function guardar() {
  ...
  const ingresoNum = Number(ingreso.value);
  ...
  gastos.total = gastos.transporte + gastos.alimentacion + gastos.otros;

  const neto = ingresoNum - gastos.total;
  ...
  const record = {
    ...
    data: {
      ...
      gastos: gastos,
      neto: neto,
      ingreso_por_hora: horas > 0 ? neto / horas : 0,
      nivel: nivel.value,
      eventos: eventos.value
    },

    breg: {
      reserva_15: ingresoNum * 0.15,
      disponible: ingresoNum - gastos.total - (ingresoNum * 0.15)
    }
  };
